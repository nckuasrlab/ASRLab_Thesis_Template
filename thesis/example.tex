...
本論文建構了一個低成本高效能的自動駕駛雛型，此雛型具備電腦視覺架構，控制系統和車輛間通訊系統，從自動汽車軟體的規範開始，在不同的抽象層上執行，直到完整實現最終目標。透過在即時運行的平台上同時整合 ROS, OpenCV, Linux 及 閉合迴路控制系統來促使軟體運行。本論文以低於 100 美元的預算下，並搭配開源軟體技術，在短時間建構了自動駕駛雛型，使後續開發人員能夠以此雛型進行有效地探索和評估。在對應的效能加速下，本論文展示了該自動駕駛雛型降低了開發時間及成本，並能使後續驗證工作更加完善。
